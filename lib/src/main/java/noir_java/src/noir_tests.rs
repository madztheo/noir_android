...
fn test_prove_verify_circuit() {
    ...
    // Set up SRS
    let _num_points = setup_srs(TEST_CIRCUIT_BYTECODE, None, false).expect("Failed to setup SRS");
    
    // Generate a proof
    let (proof, vk) = prove_ultra_honk(TEST_CIRCUIT_BYTECODE, witness_map, false)
